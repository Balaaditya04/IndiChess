...
const Board = ({ addMove }) => {
  ...
  const handleDrop = (e, row, col) => {
    ...
    if (validMoves.some(([r, c]) => r === row && c === col)) {
      ...
      // Track last move
      setLastMoveSquares({ from: [fromRow, fromCol], to: [row, col] });
        ...
        {board.map((row, rowIndex) => (
          <div key={rowIndex} className="row">
            {row.map((piece, colIndex) => {
              const isLastMoveFrom = lastMoveSquares.from && lastMoveSquares.from[0] === rowIndex && lastMoveSquares.from[1] === colIndex;
              const isLastMoveTo = lastMoveSquares.to && lastMoveSquares.to[0] === rowIndex && lastMoveSquares.to[1] === colIndex;
              const isSelected = isSquareSelected && selectedSquare && selectedSquare[0] === rowIndex && selectedSquare[1] === colIndex;
              const isWhiteKingInCheck = isKingInCheckHook[0] && piece === 'K' && whiteKingCoordinates[0] === rowIndex && whiteKingCoordinates[1] === colIndex;
              const isBlackKingInCheck = isKingInCheckHook[1] && piece === 'k' && blackKingCoordinates[0] === rowIndex && blackKingCoordinates[1] === colIndex;
              const hasValidMove = validMoves.some(([r, c]) => r === rowIndex && c === colIndex);
              const hasPieceToCapture = hasValidMove && board[rowIndex][colIndex] !== "";

              return (
                <div
                  key={colIndex}
                  className={`square ${((rowIndex + colIndex) % 2 === 0) ? 'light' : 'dark'} ${isLastMoveFrom ? 'last-move-from' : ''
                    } ${isLastMoveTo ? 'last-move-to' : ''
                    } ${isSelected ? 'selected' : ''
                    } ${isWhiteKingInCheck || isBlackKingInCheck ? 'in-check' : ''
                    }`}
                  style={{ width: `${100 / 8}%` }}
                ...
                >
                  ...
                  {hasValidMove && (
                    hasPieceToCapture ?
                      <div className="valid-move-capture"></div> :
